...
void Version::read_thread(void* args) {
   read_struct* rs = reinterpret_cast<read_struct*>(args);
   Version* version = rs->version;
   DEBUG_T("before table_cache_ get, file_number:%lld\n", 
            rs->file_number);
   rs->status = version->vset_->table_cache_->Get(rs->options, 
                rs->file_number, rs->file_size, 
                rs->ikey, &rs->saver, SaveValue);
   DEBUG_T("after table_cache_ get\n");
}
...
Status Version::Get(const ReadOptions& options,
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    ...
    for (uint32_t i = 0; i < num_files; ++i) {
      ...
      //     
      //    if(search_partner) {
      //        //ThreadPool* search_pool = new ThreadPool(2);
      //        std::string* value1 = new std::string;
      //        std::string* value2 = new std::string;
      //        read_struct* rs = new read_struct[2];
      //        for(int k = 0; k < 2; k++){
      //            rs[k].saver.state = kNotFound;
      //            rs[k].saver.ucmp = ucmp;
      //            rs[k].saver.user_key = user_key;
      //            if(k == 0)
      //              rs[k].saver.value = value1;
      //            else
      //              rs[k].saver.value = value2;
      //            rs[k].ikey = ikey;
      //            rs[k].options = options;
      //            rs[k].version = this;
      //            if(k == 0) {
      //                rs[k].file_number = ptner_search.partner_number;
      //                rs[k].file_size = ptner_search.partner_size;
      //            } else {
      //                rs[k].file_number = f->number;
      //                rs[k].file_size = f->file_size;
      //            }
      //            vset_->read_thpool_->AddJob(read_thread, &rs[k]);
      //            //search_pool->AddJob(read_thread, &rs[k]);
      //        }
      //        DEBUG_T("wait read_thpool_ finished.....\n");
      //        vset_->read_thpool_->WaitAll();
      //        //search_pool->JoinAll();
      //        DEBUG_T("read_thpool_ finished\n");
      //       
      //        if(rs[0].saver.state == kNotFound){
      //            switch(rs[1].saver.state) {
      //              case kNotFound:
...
VersionSet::VersionSet(const std::string& dbname,
  ...
  ////////////meggie
  //read_thpool_ = new ThreadPool(2, "read_thpool");
  ///////////meggie
}
