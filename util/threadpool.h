...
class ThreadPool {
    ...
    void Task() {//线程的执行体
        while( !bailout ) {
            ...
            //DEBUG_T("after notify_one, jobs_left:%d\n", 
             //       jobs_left.load(std::memory_order_relaxed));
        }
    }
...
public:
    ...
    auto AddJob(F&& f, Args&&... args)->std::future<decltype(f(args...))> {
        ...
        ++jobs_left;//剩下的还未完成的任务数量
        //DEBUG_T("after add job, jobs_left:%d\n", 
          //      jobs_left.load(std::memory_order_relaxed));
    ...
    void WaitAll() {
        if( jobs_left > 0 ) {
            ...
            //直到所有的任务都完成
            //DEBUG_T("have get lock\n");
            wait_var.wait( lk, [this]{ return this->jobs_left == 0; } );
            //DEBUG_T("threadcount:%d, wait finished\n", threadcount);
            //条件变量才能调用wait函数，表示
            lk.unlock();
            //DEBUG_T("unlock\n");
        }
